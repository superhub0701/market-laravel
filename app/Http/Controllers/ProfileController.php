...
class ProfileController extends Controller
{
    ...
    public function disableProduct($turn)
    {
        $id = auth()-> user()->id;
        try{
        if($turn == '1'){
            DB::table('products')->where('user_id', $id)->update(array('delete_status' => $turn));  
            session() -> flash('success', 'Your products has been temporary deleted.');
        }else{
            DB::table('products')->where('user_id', $id)->update(array('delete_status' => $turn));
            session() -> flash('success', 'Your products has been restored.');    
        }
        }
        catch (RequestException $e){
            session() -> flash('errormessage', $e -> getMessage());
