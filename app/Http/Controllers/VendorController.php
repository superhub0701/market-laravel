...
class VendorController extends Controller
{
    ...
    {
        $id = auth()->user()->id;
        $product = Product::where('user_id',$id)->where('delete_status','0')->paginate(config('marketplace.products_per_page'));
    ...
    public function verifyAddress($verifyAddress)
    {
        $decode = base64_decode($verifyAddress);
    ...
    {
        $id = auth()->user()->id;
        $count = Product::where('user_id',$id)->where('delete_status','1')->get()->count();
        if($count > 0){
              // session() -> flash('success', 'You can not add any product in Vacation Mode!');
              return redirect()->back()->with('success', 'You can not add any product in Vacation Mode!');   
        }
